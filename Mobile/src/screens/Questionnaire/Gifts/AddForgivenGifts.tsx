...
const AddForGivenScreen = ({ navigation, route }: ApplicationScreenProps) => {
  ...
  const GiftTextFieldsUI = (item: addGiftType) => {
    return (
      <TextField
        ...
        label={item.title}
        error={
          item.title == t('questionnaire:NAME_OF_RECIPIENT')
            ? error
  ...
  const GiftDropDownFieldsUI = item => {
    return (
      <CustomDropdown
        ...
        data={receipientTypeDataForGiven}
  ...
  const addGeneralDetails = (item: GiftData) => {
    const response: DetailPageRequest<BusinessGeneralData> = {
      entityPageID: pageId,
      pagecode: PageCode.GiftsDebtForgiveness,
      entityId: '0',
  ...
  const editGeneralDetails = (item: GiftData, entityIDUpdated: string) => {
    setFetching(true)
    item.code = PageCode.GiftsDebtForgivenessDetails
    item.entityid = entityIDUpdated
    item.isDirty = 'true'

    const request: DetailPageRequest<BusinessGeneralData> = {
      pagecode: PageCode.GiftsDebtForgivenessDetails,
  ...
  const submitGift: SubmitHandler<Record<string, string>> = async formdata => {
    const data: ForgivenData = {
      ...
      chkEntity: formdata.recipientsType == 'O' ? 'Y' : 'N',
    }
  ...
  const getScreenData = () => {
    const endPoitns = `code=${PageCode.GiftsDebtForgivenessDetails}&entityId=${entityID}`
    ...
    getNavDetailsAPI(endPoitns)
      ...
      .then(res => {
        ...
        } else if (dataMapData?.chkEntity == 'Y') {
          setSelectedRecipient(t('questionnaire:OTHER'))
        } else {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerTitle: isEdit
        ? t('questionnaire:EDIT_FORGIVIVEN')
        : t('questionnaire:ADD_FORGIVIVEN'),
      headerRight: () => (
  ...
  return (
    <SafeAreaView style={glbStyles.safeAreaView}>
      <KeyboardAwareScrollView>
        ...
        />
        <FlatList data={addForgiven} renderItem={renderAddGiftScreen} />
      </KeyboardAwareScrollView>
    </SafeAreaView>
  )
}
export default AddForGivenScreen
